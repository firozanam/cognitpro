...
export default function EditPrompt({ prompt, categories, tags }: EditPromptProps) {
  const [selectedTags, setSelectedTags] = useState<number[]>(prompt.tags.map(tag => tag.id));
  ...
  const { data, setData, put, processing, errors, reset } = useForm({
    ...
    minimum_price: prompt.minimum_price?.toString() || '5.00',
  ...
  const handleArchive = () => {
    if (confirm('Are you sure you want to archive this prompt? It will no longer be visible to buyers.')) {
      put(`/api/prompts/${prompt.uuid}`, {
  ...
  return (
    <AppLayout>
      <Head title={`Edit ${prompt.title} - CognitPro`} />
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      Click tags to add or remove them from your prompt
                    </p>
                  </div>
                </CardContent>
              </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-3 text-sm">
                <div className="flex items-start gap-2">
                  ...
                  <p>Changes to published prompts will be visible immediately</p>
                </div>
                <div className="flex items-start gap-2">
                  <AlertCircle className="h-4 w-4 text-primary mt-0.5 flex-shrink-0" />
                  <p>Consider creating a new version for major changes</p>
                </div>
                <div className="flex items-start gap-2">
                  <AlertCircle className="h-4 w-4 text-primary mt-0.5 flex-shrink-0" />
                  <p>Archiving will hide the prompt from buyers</p>
                </div>
              </CardContent>
            </Card>
          </div>
